...
`scheduler` 同时支持 **cron 表达式**与固定间隔，**填写 cron 后优先使用 cron**：

```yaml
scheduler:
  enabled: true
  cron: "0 */6 * * *"     # 每 6 小时
  timezone: Asia/Shanghai # 留空使用本机时区
  interval: 30m           # 未配置 cron 时生效
  run_on_start: true      # 服务启动时先跑一次
```
...
| 写法 | 含义 |
| --- | --- |
| `0 */6 * * *` | 5 字段：分 时 日 月 周，每 6 小时整点 |
| `30 8 * * 1-5` | 工作日 8:30 |
| `0 0 1 * *` | 每月 1 号 0 点 |
| `30 0 */6 * * *` | 可选秒的 6 字段 |
| `@every 2h`、`@daily`、`@hourly` | 描述符写法 |

管理页面可以直接填 cron、点「预览 cron」查看接下来 5 次触发时间，状态卡片会显示「下次运行」。
如果上一轮任务还没结束，本次定时触发会被跳过（日志中会说明），不会并发执行。
定时配置修改后最多 30 秒内生效，无需重启。

### 只有整轮任务成功才替换文件

一次任务会依次完成「拉取订阅 → 合并节点池 → 测速筛选 → 生成配置」，**任何一步失败都不会动现有文件**：

1. 生成的内容先做完整性校验：必须是合法 YAML、必须同时包含 `proxies` 与 `proxy-groups`、每个节点的关键字段齐全；
2. 校验通过后，把上一版复制为 `<path>.bak`（`output.keep_backup`，默认开启）；
3. 新内容写入 `<path>.tmp` 并 fsync；
4. 最后用 rename **原子替换**目标文件。

因此通过 `/clash.yaml` 拉取订阅的客户端要么读到完整的旧文件、要么读到完整的新文件，绝不会读到写了一半的内容；
如果一轮下来 0 可用节点，则日志提示并保留上一版配置。
...
| GET | `/api/schedule?cron=...&timezone=...` | 预览 cron 表达式接下来 5 次触发时间 |
