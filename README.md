...
* **Clash 订阅**：导入远程 `clash.yml`（明文或 base64），多个订阅合并为**一个节点池**，经内核做延迟测速，按国家 / 协议筛选、按延迟排序。默认只列可用节点，可在 设置 → 订阅 打开「显示不可用节点」。订阅地址的增删改查与「更新订阅」都在 设置 → **订阅配置**，订阅页只展示合并后的节点。
* **订阅备份 / 迁移**：一键把当前**可用节点**连同各自的原始定义备成**本地订阅**（机场换域名 / 封号也能继续用，可一键"用它替换"停用其它订阅）；支持 base64 / 备份文本导入，以及「复制全部地址」「复制全部节点」。
